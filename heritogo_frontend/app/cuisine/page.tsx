...
export default function CuisinePage() {
  ...
  const filteredPlats = platsTogolais.filter((plat) => {
    if (!plat?.nom) return false
    const matchesNom = plat.nom.toLowerCase().includes(searchInput.toLowerCase())
  ...
  return (
    <main className="relative min-h-screen w-full bg-base-100 text-base-content
      ...
      <div className="absolute top-10 left-1/4 w-100 h-100
                      bg-emerald-600/5 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 container mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-black tracking-tight text-base-content
            ...
            <Utensils className="text-emerald-500 h-8 w-8 animate-pulse" />
            <span>
              Saveurs du{' '}
          ...
          <p className="mt-3 text-base text-base-content/50 max-w-xl mx-auto">
            {"Explorez la richesse de notre patrimoine culinaire. Découvrez l'histoire et les secrets des meilleurs plats traditionnels togolais."}
          </p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 px-2">
          <div className="relative flex items-center bg-base-200
            ...
            <input
              ...
              placeholder="Rechercher du fufu, ablo, une sauce..."
          ...
          {searchInput && (
            <p className="text-center text-xs italic text-base-content/40
              ...
              Résultats pour :{' '}
              <span className="text-emerald-500 font-semibold">{searchInput}</span>
            </p>
          )}
        </div>
        ...
        {filteredPlats.length === 0 ? (
          <div className="text-center py-20 bg-base-200 border border-base-content/5
            ...
            <p className="text-lg text-base-content/50 font-medium">
              Aucun plat traditionnel trouvé
            </p>
            <p className="text-xs text-base-content/30 mt-1">
              Essayez une autre orthographe (ex: come, djenkoume) ou une catégorie.
            </p>
          </div>
        ) : (
            ...
            {filteredPlats.map((plat) => (
              ...
              >
                ...
                <figure className="relative w-full h-48 overflow-hidden bg-base-300">
                  ...
                  />
                  {/* Overlay image — toujours sombre sur photo */}
                  <div className="absolute inset-0 bg-linear-to-t
                                  from-black/60 to-transparent" />

                  {/* Badge catégorie */}
                  <span className={`absolute top-3 right-3 badge border-none
                                    text-white font-bold text-[10px] tracking-wider
                                    uppercase px-2.5 py-1.5 rounded-full shadow-lg
                ...
                <div className="p-5 flex flex-col justify-between grow gap-4">
                  <div className="space-y-2">
                    <h2 className="text-lg font-bold text-base-content tracking-wide
                    ...
                    <p className="inline-flex items-center gap-1 text-xs text-amber-500
                                  font-semibold bg-amber-400/10 px-2 py-0.5 rounded-md
                                  border border-amber-400/20 max-w-full">
                      <ChefHat size={12} className="shrink-0" />
                      <span className="line-clamp-1">
                        Acc. : {plat.accompagnementsIdaux}
                      </span>
                    </p>
                  ...
                  {/* Action */}
                  <div className="pt-3 border-t border-base-content/5 flex justify-end">
                    <Link href={`/cuisine/${plat.id}`} className="w-full">
                      <button className="btn btn-sm btn-block rounded-xl border-none
                                         bg-base-content/5
